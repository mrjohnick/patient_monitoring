...
export const breathGraph2 = [15, 18, 20, 23, 25, 25, 16, 15, 18, 17];
export const oxygenGraph2 = [90, 87, 89, 90, 90, 88, 85, 88, 89, 90];

const Room2 = ({ navigation }) => {
  ...
  function avg(heartGraph2) {
    var sum = 0;
    heartGraph2.forEach(function (item, idx) {
      sum += item;
    });
    return sum / heartGraph2.length;
  }
  ...
  const handleHeart = () => {
    setHeart(randomNumberInRange(90, 120));
  };

  const handleBreath = () => {
    setBreath(randomNumberInRange(15, 25));
  };

  const handleOxygen = () => {
    setOxygen(randomNumberInRange(85, 90));
  };
  ...
  const room2Handler = async (heart, breath, oxygen) => {
    const patientsDoc = doc(db, "patients", "room2");
  ...
  useEffect(() => {
    room2Handler(heart, breath, oxygen);
  }, [heart, breath, oxygen]);

  // Listen for change in heart, breath, and oxygen then add to the const graph
  useEffect(() => {
    add(heartGraph2, heart);
  }, [heart]);

  useEffect(() => {
    add(breathGraph2, breath);
  }, [breath]);

  useEffect(() => {
    add(oxygenGraph2, oxygen);
  }, [oxygen]);
          ...
          <ScrollView>
            ...
            <View style={styles.patientInformation}>
              ...
              <View style={styles.patientNameWrapper}>
                <Text style={styles.patientName}>Bobby Rob</Text>
              </View>
              {/* Patient date of admission */}
              <View style={styles.patientDateWrapper}>
                <Text style={styles.patientDate}>
                  Date of admission: {patientData[0].dateOfAdmission}
                </Text>
              </View>
            </View>
